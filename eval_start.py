...
def model_answers(target_model, qa_file, batch_size):
    ...
    eval_dataset = load_dataset(
        os.path.dirname(qa_file), data_files=qa_file, split="train"
    )
    data_loader = DataLoader(
        dataset=eval_dataset, batch_size=batch_size, shuffle=False, num_workers=8
    )
...
def model_judges(judge_func, answer_file, jbatchsize):
    ...
    eval_dataset = load_dataset(
        os.path.dirname(answer_file), data_files=answer_file, split="train"
    )
    data_loader = DataLoader(
        dataset=eval_dataset, batch_size=jbatchsize, shuffle=False, num_workers=8
    )
    ...
    with open(judge_file, "w", encoding="utf-8") as f:
        for i in tqdm(data_loader, total=len(data_loader)):
            ...
            for q, ya, ra in zip(i["question"], i["your_answer"], i["right_answer"]):
                ...
                prompts += [
                    template.format(question=q, your_answer=ya, right_answer=ra)
                ]
            ...
            for q, ya, ra, r in zip(
                i["question"], i["your_answer"], i["right_answer"], responses
            ):
...
def select_model():
    ...
    parser.add_argument(
        "--qafile", type=str, default="byllm/qa_data_ready.jsonl", help="qa file path"
    )
    parser.add_argument(
        "--afile",
        type=str,
        default="byllm/qa_data_ready_llm_answers.jsonl",
        help="answer file path",
    )
    parser.add_argument("--batchsize", type=int, default=50, help="batch size")
    parser.add_argument(
        "--jbatchsize", type=int, default=50, help="batch size of judge"
    )
    parser.add_argument(
        "--provider", type=str, default=None, help="model from which provider"
    )
    parser.add_argument(
        "--jprovider", type=str, default=None, help="judge from which provider"
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="max new tokens generated by judge model",
    )
    ...
    if (not target_model_config) and args.model:
        print(
            f"{args.model} API not found, will be using huggingface model {args.model} as target model. If you still mean API, please add this model in modelsAPI.py, api-config.json, eval_start.py."
        )
        ...
        if target_model_config["provider"] == "baidu":
            target_model = modelsAPI.baidu_factory(
                target_model_config["api_key"],
                target_model_config["secret_key"],
                args.max_new_tokens,
                target_model_config["base_url"],
            )
        elif target_model_config["provider"] == "deepseek":
            target_model = modelsAPI.deepseek_factory(
                target_model_config["api_key"],
                args.max_new_tokens,
                target_model_config["base_url"],
            )
            ...
            raise Exception(
                f"Provider {target_model_config['provider']} not supported, please add the support in modelsAPI.py and eval_start.py."
            )

    if (not model_judge_config) and args.judge:
        print(
            f"{args.judge} API not found, will be using huggingface model {args.judge} as judge. If you still mean API, please add this model in modelsAPI.py, api-config.json, eval_start.py."
        )
        ...
        if model_judge_config["provider"] == "baidu":
            model_judge = modelsAPI.baidu_factory(
                model_judge_config["api_key"],
                model_judge_config["secret_key"],
                args.jmax_new_tokens,
                model_judge_config["base_url"],
            )
        elif model_judge_config["provider"] == "deepseek":
            model_judge = modelsAPI.deepseek_factory(
                model_judge_config["api_key"],
                args.jmax_new_tokens,
                model_judge_config["base_url"],
            )
            ...
            raise Exception(
                f"Provider {model_judge_config['provider']} not supported, please add the support in modelsAPI.py and eval_start.py."
            )
...
if __name__ == "__main__":
    ...
    if model_judge:
        right_a, all_q, accuracy, network_err, judge_file = model_judges(
            model_judge, qa_file_llm_answers, args.jbatchsize
        )
    print(
        f"right answers ={right_a}, successful judges = {all_q}, accuracy ={accuracy}. network errors = {network_err}. Results are saved to {judge_file}"
    )
